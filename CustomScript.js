...
const EnableLeveling = false;				//CHANGE to true and open the store to level up characters, do the opposite to enable unlock all
...
if(!System.IO.Directory.Exists(MarketUpdaterPath)) System.IO.Directory.CreateDirectory(MarketUpdaterPath);	//DO NOT TOUCH THIS LINE OF CODE
...
static function OnBeforeRequest(oSession: Session) {
    ...
    if(!EnableLeveling && Bloodweb_v7 && (oSession.uriContains("api/v1/dbd-character-data/bloodweb") || oSession.uriContains("api/v1/dbd-character-data/get-all"))){
        try{
            ...
            if (customStatus) {
                ...
                var customDataJsonFixed = {};
                for (var i = 0; i < customDataJson["List"].Count; i++) {
                    var character = customDataJson["List"][i];
                    customDataJsonFixed[character[jCharacterName]] = {
                        PrestigeLevel: character[jPrestigeLevel]
                        };
                }	
            }
            ...
            if (oSession.uriContains("api/v1/dbd-character-data/get-all")){
                ...
                if (customStatus){
                    for(var i=0;i<getallJson[jList].Count;i++){
                        try{
                            getallJson[jList][i][jPrestigeLevel] = customDataJsonFixed[getallJson[jList][i][jCharacterName]][jPrestigeLevel];
                        }
                        catch(e){
                            getallJson[jList][i][jPrestigeLevel] = 9;
                        }
                    }
            ...
            bloodwebJson[jCharacterName] = characterSelected;
            try{
                if (customStatus && customDataJsonFixed[characterSelected][jPrestigeLevel] != null){ bloodwebJson[jPrestigeLevel] = customDataJsonFixed[characterSelected][jPrestigeLevel]; }
            }
            catch(e){
                bloodwebJson[jPrestigeLevel] = 9;
            }
            oSession.utilSetResponseBody(Fiddler.WebFormats.JSON.JsonEncode(bloodwebJson));
            oSession.responseCode = 200;
        }
        catch(e){
            FiddlerObject.log(e);
    ...
    if (!QuestBlock && Quest && oSession.uriContains("api/v1/gameDataAnalytics/v2/batch")){
        try{
			...
			var matchPath = MarketUpdaterPath + "MatchIds.json";
			if (!System.IO.File.Exists(questPath) || !System.IO.File.Exists(headersPath)) return;
            var matchId = "";
			var krakenMatchId = "";
			if (System.IO.File.Exists(matchPath)){
				var matchJson = Fiddler.WebFormats.JSON.JsonDecode(System.IO.File.ReadAllText(matchPath)).JSONObject;
				matchId = matchJson["matchId"];
				krakenMatchId = matchJson["krakenMatchId"];
			}
			else System.IO.File.WriteAllText(matchPath, '{"matchId":"","krakenMatchId":""}')
			oSession.utilDecodeRequest();
			...
			if(matchId !== "" && matchJson["matchId"] != matchId){
				matchJson["matchId"] = matchId;
				System.IO.File.WriteAllText(matchPath, Fiddler.WebFormats.JSON.JsonEncode(matchJson));
			}
			if(krakenMatchId !== "" && matchJson["krakenMatchId"] != krakenMatchId){
				matchJson["krakenMatchId"] = krakenMatchId;
				System.IO.File.WriteAllText(matchPath, Fiddler.WebFormats.JSON.JsonEncode(matchJson));
			}
			FiddlerObject.log("matchId: " + matchId);
			FiddlerObject.log("krakenMatchId: " + krakenMatchId);
			FiddlerObject.log("check: " + check);
...
static function OnBeforeResponse(oSession: Session) {
    ...
    if (Market_v4 && oSession.uriContains("/api/v1/dbd-inventories/all")) {
        try{		
            var marketPath = "";
            if(EnableLeveling) marketPath = MarketUpdaterPath + "Files\\MarketNoSavefile.json";
            else marketPath = MarketUpdaterPath + "Files\\MarketWithPerks.json";
